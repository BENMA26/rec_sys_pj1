...
class ShareBottomModel(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.test_ctr_auc.update(ctr_preds, ctr_labels.long())
        self.test_cvr_auc.update(cvr_preds, cvr_labels.long())
    ...
    def on_test_epoch_end(self):
        """测试 epoch 结束时：计算并记录 AUC"""
        ctr_auc = self.test_ctr_auc.compute()
        cvr_auc = self.test_cvr_auc.compute()

        self.log('test_ctr_auc', ctr_auc, prog_bar=True, sync_dist=True)
        self.log('test_cvr_auc', cvr_auc, prog_bar=True, sync_dist=True)
...
class MOEModelWithTorchJD(pl.LightningModule):
    """MOE 多任务学习模型 - 共享单门控 + TorchJD 梯度聚合"""
